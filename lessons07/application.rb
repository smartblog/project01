...
class Application
  ...
  def list_trains_on_station
    ...
    Station.all[station_number].all_trains{|train| puts "Train: #{train.number}, Type: #{train.train_type}, CarriagesCount: #{train.carriages.count}"}
    rescue => e
      puts e.message
  end
  ...
  def list_carriages_of_train
    ...
    Train.all[train_number].all_carriages{|carriage, index| puts "#{index} - Type: #{carriage.carriage_type}, BusyPlaces: #{carriage.busy_places}, FreePlaces: #{carriage.free_places}"}
    elsif Train.all[train_number].train_type == "cargo"
    Train.all[train_number].all_carriages{|carriage, index| puts "#{index} - Type: #{carriage.carriage_type}, BusyAmount: #{carriage.busy_amount}, FreeAmount: #{carriage.free_amount}"}
    end
    rescue => e
      puts e.message
  end
